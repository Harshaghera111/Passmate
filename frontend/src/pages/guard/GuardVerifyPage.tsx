...
const GuardVerifyPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    passApi.verify(id)
      ...
      .catch(err => {
        console.error(err);
        toast.error("Failed to verify pass. It may be invalid or not found.");
      })
  ...
  const completeAction = async (type: 'exit' | 'entry') => {
    ...
    try {
      if (type === 'exit') {
         await passApi.scanExit(id);
         toast.success('Exit Marked Successfully!');
      } else {
         const res = await passApi.scanEntry(id);
         if (res.isLate) {
        ...
        <div className={`bg-white rounded-3xl p-6 shadow-xl border-2 flex flex-col items-center text-center -mt-16 relative z-20 ${isValid ? 'border-emerald-100' : 'border-red-100'}`}>
          ...
          <div className="w-full bg-bg-muted mt-6 rounded-2xl p-4 text-left border border-border">
            <div className="mb-3 border-b border-border pb-3">
              ...
              <p className="font-bold text-sm">{pass.reason_detail || pass.reasonDetail}</p>
            </div>
            <div className="mb-3 border-b border-border pb-3">
              <p className="text-[10px] font-bold uppercase text-text-muted mb-0.5">Approved By</p>
              <p className="font-bold text-sm text-purple-700">{pass.wardenName || 'Warden'}</p>
            </div>
            <div>
              ...
              <p className="font-bold text-lg text-amber-700 tracking-tight">
                {new Date(pass.expected_return || pass.expectedReturn || '').toLocaleString(undefined, {weekday:'short', hour:'numeric', minute:'numeric'})}
              </p>
            </div>
          </div>
        </div>
