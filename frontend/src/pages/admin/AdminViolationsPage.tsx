...
import { AlertTriangle, ShieldAlert, Filter, Mail, ShieldCheck, Loader2, RefreshCw } from 'lucide-react';
...
const AdminViolationsPage: React.FC = () => {
  ...
  const fetchViolations = async () => {
    ...
    setError(null);
    try {
      const data = await adminApi.violations();
      setViolators(data as Violator[]);
    } catch (err: unknown) {
  ...
  const filteredViolators = violators
    .filter(v => filter === 'severe' ? v.violations >= 3 : true)
  ...
  return (
    <div className="space-y-6 page-enter max-w-6xl mx-auto h-[calc(100vh-100px)] flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 flex-shrink-0">
        ...
        <button onClick={fetchViolations} className="btn btn-secondary text-sm gap-2">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm flex flex-col flex-1 min-h-0">
        ...
        <div className="p-4 border-b border-border flex justify-between gap-4 flex-shrink-0 bg-gray-50/50">
          <div className="flex gap-2">
            ...
            >
              All Violators ({violators.length})
            </button>
            <button 
            ...
            >
              <ShieldAlert size={14} className={filter === 'severe' ? 'text-red-500' : ''} />
              Severe 3+ ({violators.filter(v => v.violations >= 3).length})
            </button>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6 bg-bg-base/50">
          ...
          {loading && (
            <div className="flex items-center justify-center h-48 gap-3 text-text-muted">
              ...
              <span className="font-medium">Loading violation reports...</span>
            </div>
          )}
          ...
          {error && !loading && (
            <div className="flex flex-col items-center justify-center h-48 gap-3 text-red-600">
              ...
              <p className="font-semibold">Failed to load violations</p>
              <p className="text-sm text-text-muted">{error}</p>
              <button onClick={fetchViolations} className="btn btn-secondary text-sm mt-2">Try Again</button>
            </div>
          )}
          ...
          {!loading && !error && (
            <div className="space-y-4">
              {filteredViolators.map(student => {
                ...
                const lateReturns = student.total_passes - student.on_time_returns;
                return (
                  <div key={student.id} className={`bg-white rounded-xl p-5 sm:p-6 border shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center justify-between transition-colors ${isSevere ? 'border-red-200 shadow-red-500/5' : 'border-amber-200 shadow-amber-500/5'}`}>
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-text-muted mt-0.5 font-mono">
                          {student.usn} • {student.room} {student.block ? `(${student.block})` : ''} {student.hostel ? `• ${student.hostel}` : ''}
                        </p>
                        
                        <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 text-sm">
                          <div>
                            <span className="text-text-muted uppercase text-[10px] font-bold block mb-0.5">Recorded Violations</span>
                            <span className={`font-bold text-lg ${isSevere ? 'text-red-600' : 'text-amber-600'}`}>{student.violations}</span>
                          </div>
                          <div>
                            <span className="text-text-muted uppercase text-[10px] font-bold block mb-0.5">Total Passes</span>
                            <span className="font-bold text-text-primary">{student.total_passes}</span>
                          </div>
                          <div>
                            <span className="text-text-muted uppercase text-[10px] font-bold block mb-0.5">Late Returns</span>
                            <span className="font-bold text-accent-danger">{lateReturns >= 0 ? lateReturns : '—'}</span>
                          </div>
                          <div>
                            <span className="text-text-muted uppercase text-[10px] font-bold block mb-0.5">On-Time Rate</span>
                            <span className="font-bold text-emerald-600">
                              {student.total_passes > 0 ? Math.round((student.on_time_returns / student.total_passes) * 100) : 0}%
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="w-full md:w-auto flex flex-col gap-2 border-t md:border-t-0 border-border pt-4 md:pt-0">
                      <button className="btn btn-secondary w-full md:w-auto text-sm justify-start md:justify-center">
                        <Mail size={16} /> Notify Parent ({student.parent_mobile || '—'})
                      </button>
